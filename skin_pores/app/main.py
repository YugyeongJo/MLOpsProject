...
model.load_state_dict(torch.load('model/best_vgg18_model_pores.pth', map_location=device))
model.eval()
    ...
    try:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                pores_prob = float(probabilities[0] * 100)
                normal_prob = float(probabilities[1] * 100)

     
                class_probabilities = {
                    "pores": round(pores_prob, 1),
                    "normal": round(normal_prob, 1)
                }


                print("\n피부 홍조 분석 결과:")
                if pores_prob > normal_prob:
                    prediction = "flushing"
                    print("홍조가 있습니다.")
                else:
                    prediction = "normal"
                    print("홍조가 없습니다.")
                print(f"확률: 홍조 {pores_prob:.1f}% / 정상 {normal_prob:.1f}%")
