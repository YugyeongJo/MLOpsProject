...
model.load_state_dict(torch.load('model/best_vgg18_model_wrinkle.pth', map_location=device))
model.eval()
    ...
    try:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                wrinkle_prob = float(probabilities[0] * 100)
                normal_prob = float(probabilities[1] * 100)

     
                class_probabilities = {
                    "wrinkle": round(wrinkle_prob, 1),
                    "normal": round(normal_prob, 1)
                }


                print("\n피부 주름 분석 결과:")
                if wrinkle_prob > normal_prob:
                    prediction = "wrinkle"
                    print("주름 있습니다.")
                else:
                    prediction = "normal"
                    print("주름이 없습니다.")
                print(f"확률: 주름 {wrinkle_prob:.1f}% / 정상 {normal_prob:.1f}%")
